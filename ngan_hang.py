...
# 2. Các đường dẫn tiềm năng chứa plugins
possible_paths = [
    os.path.join(qt_root, "Qt6", "plugins"), # Cấu trúc chuẩn
    os.path.join(qt_root, "plugins"),        # Cấu trúc rút gọn
    os.path.join(sys.prefix, "lib", "python3.11", "site-packages", "PyQt6", "Qt6", "plugins") # Đường dẫn venv
]

# 3. Tìm đường dẫn hợp lệ
plugin_path = None
for path in possible_paths:
    if os.path.exists(os.path.join(path, "platforms", "libqcocoa.dylib")):
        plugin_path = path
        break

# 4. Thiết lập biến môi trường nếu tìm thấy
if plugin_path:
    os.environ["QT_PLUGIN_PATH"] = plugin_path
    print(f"✅ Đã tìm thấy Qt Plugins tại: {plugin_path}")
else:
    # Fallback: Thử set đường dẫn mặc định dù không tìm thấy file (hy vọng hệ thống tự fix)
    fallback = os.path.join(qt_root, "Qt6", "plugins")
    os.environ["QT_PLUGIN_PATH"] = fallback
    print(f"⚠️ Không tìm thấy plugin cocoa. Đang thử fallback: {fallback}")
